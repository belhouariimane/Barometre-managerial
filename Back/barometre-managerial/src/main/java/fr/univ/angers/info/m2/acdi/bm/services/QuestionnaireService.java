...
public class QuestionnaireService {
	...
	public ResponseSingleQuestionnaire insertOne(Questionnaire questionnaire) {
		...
		try {
			administrateurRepository.findById(questionnaire.getAdministrateur().getId())
					.orElseThrow(() -> new ResourceNotFoundException(Administrateur.class.getSimpleName(), "id",
							questionnaire.getAdministrateur().getId()));
		...
		if (questionnaire.getId() != null) {
			return new ResponseSingleQuestionnaire(ConstantesREST.ID_NOT_INSERTABLE, null, HttpStatus.BAD_REQUEST);
		}
